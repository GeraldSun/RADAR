...
def composite_score(df: pd.DataFrame) -> float:
    """
    Per-row score: 1 if agreement matches AND both severity and edit_type match, else 0.
    Returns mean over rows where all gold labels are present.
    """
    req = df["agreement"].notna() & df["agreement_pred"].notna()
    dfc = df.loc[req].copy()
    if len(dfc) == 0:
        return float("nan")

    agree_match = dfc["agreement"] == dfc["agreement_pred"]
    sev_ok = (
        dfc["severity"].notna()
        & dfc["severity_pred"].notna()
        & (dfc["severity"] == dfc["severity_pred"])
    )
    typ_ok = (
        dfc["edit_type"].notna()
        & dfc["edit_type_pred"].notna()
        & (dfc["edit_type"] == dfc["edit_type_pred"])
    )
    comp = (agree_match & sev_ok & typ_ok).astype(int)
